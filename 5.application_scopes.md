...
There are two kinds of application scope types:
 ...
 - **Extended application scope types**: Extended application scopes are optional per runtime, meaning that each runtime may choose which extended scopes are supported. It is expected that runtimes will implement extended scopes as they need. In this version of the spec, allowing user-defined extended application scope types is not supported.
...
The following core scopes are defined in our current OAM specification.
...
Here is the [definition](core/scopes/network_scope.md) of the network application scope.
...
Here is the [definition](core/scopes/health_scope.md) of the health application scope.
