...
The core workload types MUST all be supported by any runtime implementation of this specification.
...
A service is used for long-running, scalable workloads that have an endpoint to respond to network traffic. Common use cases include web applications and API servers. The service workload type has the following characteristics:
 - Defines a container runtime where zero or more replicas of the same container may be run at a time. 
 - An application operator can scale a service in or out by increasing or decreasing the replica count by configuring scaling traits. 
 - A service is a long running process. A runtime MUST restart replicas that exit regardless of error code.
 - A service has an endpoint with an automatically assigned virtual IP address and DNS name addressable within the network scope to which the component belongs.
...
A singleton service is a special case of the Service workload type that is limited to at most one replica. The singleton service workload type as the following characteristics:
 ...
 - A singleton service is a long running process. A runtime MUST restart the singleton service replica if it exits regardless of error code.
 - A service has an endpoint with an automatically assigned virtual IP address and DNS name addressable within the network scope to which the component belongs.
