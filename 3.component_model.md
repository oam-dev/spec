...
3. Whether they receive traffic on an endpoint. Workload types that have an endpoint are addressable through a DNS name within their network scope and can be assigned traffic routing traits.
...
|Name|Type|Has endpoint|Replicable|Daemonized|
|-|-|-|-|-|
...
A worker is used for long-running, scalable workloads that do not have an endpoint to respond to network traffic. Workers are typically used to pull from queues or provide other offline processing. The worker workload type has the following characteristics:
 - Defines a container runtime where zero or more replicas of the same container may be run simultaneously. 
