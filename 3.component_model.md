...
| `imagePullCredentials` | `string` | N | | Key that can be used to retrieve the credentials for pulling this secret. |

The details of the way that a runtime takes `imagePullCredentials` and loads credentials is left to the Hydra runtime implementation. For example, a Kubernetes implementation may treat this as a key that can be loaded from a secret.
