...
The application model defines the following:
 ...
 - _WorkloadDefinitions_ define the different schematics that a component can use to describe its content.
 - _Components_ represent a runnable unit, together with a description of what can be overridden during runtime.
 ...
 - _Application configuration_ describes a set of component instances, their traits, and the application scopes in
  which they are placed, combined with configuration parameters and metadata.
...
A platform that implements the Open Application Model (OAM) supplies a runtime (or runtimes) that can execute
 components. Each type of component that can be run by the OAM runtime is represented by a _workloadDefinition_ that
  describes the following:

- Metadata: Information about the workload kind
- Schema reference: A reference to a schema that defines the workload. The schemas are mostly provided by the OAM
 platform.  

### Component
 
__Component__ permit __developers__ to declare the operational characteristics of the code they deliver _in
 infrastructure neutral terms_. Apart from not burdening developers with infrastructural concerns, this frees operators and runtimes to meet a component's infrastructural needs in whatever opinionated manner they see fit.
...
- Schema reference: A reference to a schema that defines the trait
...
- Schema reference: A reference to a schema that defines the scope
...
- Scope settings: Setup application scopes
- Parameter Overrides: Values to supply to named parameters in the components
...
| ------------- |-------------|
|[1. Purpose And Goals](1.purpose_and_goals.md)| [3. Worklaod Definition](3.workload.md)|
