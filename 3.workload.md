...
Workload definitions are provided by the platform (just as traits are) so that users may inspect the platform and
 learn what workloads are available for use. Workload definitions are not extensible to platform users (only to platform
  operators). Thus, platform users MUST NOT be allowed to create new workload kinds.

## Top-Level Attributes

The following attributes are common across all schemata defined in this document. This structure is designed to allow
 object type to be determined by scanning two fields (`apiVersion` and `kind`), and provide access to common fields
...
The specification section contains a reference to the workload schema
...
DefinitionRef defines where to find the workload schema

| Attribute | Type | Required | Default Value | Description |
|-----------|------|----------|---------------|-------------|
| `name` | `string` | Y | | The workload schema's name. Must be formatted in a way that can uniquely identify a specific version of the definition  |
...
We recommend that the `name` here follows the format described in [the Group, Version, Kind section of part 2](2.overview_and_terminology.md).

We also recommend that the `name` of the workload (in the metadata section) to include the Group and Kind of the schema.

## Workload Kinds

There are three kinds of workloads:

__Core workloads__ are the kind of workloads that every OAM runtime MUST implement according to spec definition

__Standard workloads__ are the kind of workloads that a OAM runtime can choose to implement. However, the
 implementation MUST strictly adhere to the schematics defined in the spec 

__Extended workloads__ OAM platforms are free to define extra workload kinds and  

### Core Workload Kind

A core workload MUST be in the `core.oam.dev` group.

A core workload MUST all be supported according to the definitions in this section by any runtime implementation of
 this specification.
...
## Core Workload
The following core workload kinds are defined by this specification

### Container Workload
A containerized workload is used for long-running workloads. Here is the definition of the containerized workload

```yaml
apiVersion: core.oam.dev/v1alpha2
kind: workloadDefinition
metadata:
  name: containerWorkload.core.oam.dev
spec:
  definitionRef:
    name: core.oam.dev/v1alpha2.containerWorkload
```

Here is the schema of the containerized workload. As mentioned in [`DefinitionRef`](#DefinitionRef), it contains
 [the Group, Version, Kind](2.overview_and_terminology.md) fields to identify a specific version of the schema.
