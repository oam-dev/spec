...
|Application Metadata|`core.hydra.io/v1alpha1.ApplicationMetadata` | This scope groups components into a logical application, which provides metadata for that application scope. The scope does not have any functional impact on components and can therefore be used to describe groupings independent of functionality. |
...
The application metadata scope groups components into a logical application, which provides metadata for that application scope. The scope does not have any functional impact on components and can therefore be used to describe groupings independent of functionality.

The application metadata scope serves the purpose of describing crosscutting metadata for a set of components independently of how they where deployed. The metadata could provide information such as: Ownership, name, tenant etc. The scope is useful for queries across a larger set of components.

#### Example
This is an example of an application metadata scope definition.
...
metadata:
  ...
  annotations:
    version: v1.0.0
    description: "Production Corp CMS application"
spec:
  type: core.hydra.io/v1.ApplicationMetadataScope
  allowComponentOverlap: true
  parameters:
    - name: name
      ...
      required: true
    - name: owners
      description: Application owners e-mail
      type: string
      required: true
```
