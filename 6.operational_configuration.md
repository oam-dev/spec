...
- Component instance definition that defines how an instance of a component spec should be deployed. This section itself has three parts:
  - Overrides of compoment parameters
...
| Attribute | Type | Required | Default Value | Description |
|-----------|------|----------|---------------|-------------|
| `parameters` | [`[]Parameter`](#parameter) | N | | Parameters for this config that can be overridden at deployment time. |
| `scopes` | [`[]Scope`](#scope) | N | | Application scope definitions. |
| `components` | [`[]Component`](#component) | N | | Component instance definitions. |


### Parameter
The Parameters section defines all of the configurable parameters for this operational configuration.
...
| `parameterValues` | [`[]ParameterValue`](#parameterValue) | N | | Overrides of parameters that are exposed by the trait type defined in `type`. |
...
| `fromParam` | `string` | N || The application parameter whose value should be substituted into this variable as a value |
...
The `parameterOverride` section of the ops config specifies parameters that can be overridden at deployment time.
...
Here is how the previous operational configuration looks with an `ingress` trait attached to the front-end compoment:
