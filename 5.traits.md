...
Traits are defined with schematics like components. Unlike component schematics, however, each trait definition defines its own unique properties object schema to define the configuration options for the trait.  
...
| `properties` | [`Properties`](#properties) | N | | The component's configuration options. |

### Properties

The properties that this trait exposes to operators. 
...
Properties are validated against the schema appropriate for the trait.
...
`core.hydra.io/v1alpha1.ManualScaler`
...
| Attribute | Type | Required | Default Value | Description |
|-----------|------|----------|---------------|-------------|
| `replicaCount` | `int` | Y | `0` | The target number of replicas to create for a given component instance. |
