...
When an application instance is first created, it is in its _initial release state_. Each time an upgrade operation occurs, we say that a new _release_ of that instance occurs. (There is, however, no assumption that there is a corresponding `release` object stored somewhere).

### Releases

In 12 factor applications, a release is defined as a [build plus a set of configs](https://12factor.net/build-release-run). That is, any change to either a build or configuration produces a new release. In Hydra, the analog is that component, trait, and scope definitions combined with an application configuration jointly constitute a release.

For Hydra applications, release is defined thus:
...
To accommodate this definition of a release, a Hydra platform SHOULD make the following assumptions:
...
- Any change to an [application configuration](4.application_configuration.md) results (conceptually) in a new release that supersedes older releases.
  ...
  * Scopes SHOULD be applied or not applied according to the same guidelines
