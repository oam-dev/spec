...
| `image` | `string` | Y | | A path-like or URI-like representation of the location of an OCI image. Where applicable, this MAY be prefixed with a registry address. |
| `tag` | `string` | Y | latest | A string representing the tag of the OCI image. |
...
spec:
  ...
  - name: my-twitter-bot-frontend
    image: krancour/my-twitter-bot-frontend
    tag: 1.0.0
    resources:
      cpu:
        required: 1.0
...
spec:
  ...
  - name: my-twitter-bot-backend
    image: krancour/my-twitter-bot-backend
    tag: 1.0.0
    resources:
      cpu:
        required: 1.0
